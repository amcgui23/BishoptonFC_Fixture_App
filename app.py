...
def istarget(x):
    # Flexible matching for Cup variants like "Bishopton FC Black", "Bishopton Black 2014", etc.
    n = norm(x)
...
def parse_matches(url, competition):
    ...
    return df.sort_values("date").reset_index(drop=True), {"url": final_url, "http": status, "parsed": len(df)}
...
with col_tbl:
    ...
    tbl_data = calculated_table(league)
    st.dataframe(
        tbl_data,
        hide_index=True,
...
if not cup_found:
    st.info("No Scottish Cup or League Cup fixtures were detected for Bishopton FC Black in the current web scrape. Check the Diagnostics tab in the sidebar to verify source responses.")
